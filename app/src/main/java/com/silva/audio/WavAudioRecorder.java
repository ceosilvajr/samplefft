...
public class WavAudioRecorder {
    ...
    public WavAudioRecorder(boolean uncompressed, int audioSource, int sampleRate, int channelConfig,
                            int audioFormat, TextView textView) {
    ...
    private double calculate(int amplitude) {
        final double frequency = bSamples * amplitude / (double) bufferSize;
        Handler handler = new Handler(Looper.getMainLooper());
        handler.post(new Runnable() {
            @Override
            public void run() {
                mTextView.setText("" + frequency);
            }
        });
        return frequency;
    }
